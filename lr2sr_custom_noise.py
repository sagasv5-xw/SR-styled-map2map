import numpy as np
import torch
from map2map.models import styled_srsgan_custom_noise
from utils import *
...
parser.add_argument('--gen_id',action='store_true', help='generate id')
parser.add_argument('--noise-dir', default='./noise', type=str, help='directory of noise')
...
n_split = args.nsplit
gen_id = args.gen_id
noise_padding_list = [3, 4, 3, 4, 3, 4]
noise_size_list = [64, 128, 128, 256, 256, 512]
# --------------------------
# paths
...
style = np.load(args.style_path) # style in shape of (1,)
style_size = style.shape[0]
# --------------------------
...
# load model
model = styled_srsgan_custom_noise.G(in_channels, out_channels, style_size, upsample_fac)
...
for idx in range(0, tot_reps):
    ...
    noise_list = []
    for size, padding in zip(noise_size_list, noise_padding_list):
        noise = crop_noise(size, idx, padding, noise_dir=noise_dir)
        noise_list.append(noise)
    chunk_disp, chunk_vel = sr_field_custom_noise(model, lr_chunk, style, tgt_size, noise_list)
